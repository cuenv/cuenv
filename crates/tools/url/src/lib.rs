...
impl UrlToolProvider {
    ...
    fn preferred_binary_names(dir: &Path) -> Vec<String> {
        ...
        if let Some(name) = Self::dir_name(dir) {
            ...
            if Self::looks_like_version_dir(&name)
                && let Some(parent_name) = dir.parent().and_then(Self::dir_name)
                && parent_name != name
            {
    ...
    fn executable_files(dir: &Path) -> Result<Vec<PathBuf>> {
        let files = Self::sorted_files(dir)?;
    ...
    fn find_main_binary(&self, dir: &Path) -> Result<PathBuf> {
        let preferred_names = Self::preferred_binary_names(dir);

        // First, look for binaries in bin/
        let bin_dir = dir.join("bin");
        if bin_dir.exists() {
            if let Some(path) =
                Self::select_preferred_binary(Self::sorted_files(&bin_dir)?, &preferred_names)
            {
                return Ok(path);
            }
        }
        ...
        if let Some(path) =
            Self::select_preferred_binary(Self::executable_files(dir)?, &preferred_names)
        {
            return Ok(path);
        }
