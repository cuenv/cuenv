...
impl HookExecutor {
    ...
    pub async fn execute_hooks(&self, hooks: Vec<Hook>) -> Result<Vec<HookResult>> {
        ...
        info!("Starting execution of {} hooks", hooks.len());
        
        for (index, hook) in hooks.into_iter().enumerate() {
            info!("Executing hook {}/{}: {}", index + 1, results.capacity() + 1, hook.command);
            ...
            if failed && self.config.fail_fast {
                warn!("Hook {} failed, stopping execution (fail_fast enabled)", index + 1);
                break;
            }
        }
    ...
    pub async fn execute_single_hook(&self, hook: Hook) -> Result<HookResult> {
        ...
        debug!("Executing command: {} {:?} in {}", hook.command, hook.args, hook.dir.display());
    ...
    pub async fn execute_with_summary(&self, hooks: Vec<Hook>) -> Result<ExecutionSummary> {
        ...
        let completed = results.iter().filter(|r| matches!(r.status, HookStatus::Completed)).count();
        let failed = results.iter().filter(|r| matches!(r.status, HookStatus::Failed(_))).count();
        let pending = results.iter().filter(|r| matches!(r.status, HookStatus::Pending)).count();
