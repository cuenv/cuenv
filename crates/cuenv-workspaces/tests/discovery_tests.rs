...
fn test_package_json_object_style_workspaces() {
    ...
    let discovery = PackageJsonDiscovery;
    let workspace = discovery.discover(root).expect("Should discover workspace with object-style config");
...
fn test_package_json_member_missing_manifest_ignored() {
    ...
    let discovery = PackageJsonDiscovery;
    let workspace = discovery.discover(root).expect("Should discover workspace and ignore invalid members");
...
fn test_pnpm_workspace_malformed_yaml() {
    ...
    std::fs::write(root.join("pnpm-workspace.yaml"), "packages: [unclosed bracket").unwrap();
...
fn test_cargo_toml_missing_workspace_section_returns_empty() {
    ...
    let discovery = CargoTomlDiscovery;
    let workspace = discovery.discover(root).expect("Should discover workspace even without [workspace] section");
...
fn test_cargo_toml_missing_workspace_section_find_members_returns_empty() {
    ...
    let discovery = CargoTomlDiscovery;
    let members = discovery.find_members(root).expect("Should return empty members for single-package repo");
...
fn test_nested_workspace_patterns() {
    ...
    let app_path = root.join("packages").join("group1").join("apps").join("app-a");
    ...
    let app_b_path = root.join("packages").join("group2").join("apps").join("app-b");
    ...
    let lib_path = root.join("packages").join("group1").join("libs").join("lib-a");
    std::fs::create_dir_all(&lib_path).unwrap();
    std::fs::write(lib_path.join("package.json"), r#"{"name": "lib-a"}"#).unwrap();
    
    let discovery = PackageJsonDiscovery;
    let workspace = discovery.discover(root).expect("Should discover nested workspaces");
...
fn test_pnpm_mixed_exclusions() {
    ...
    let discovery = PnpmWorkspaceDiscovery;
    let workspace = discovery.discover(root).expect("Should discover workspace with mixed exclusions");
...
fn test_integration_real_cargo_toml() {
    ...
    if !root.join("Cargo.toml").exists() {
        println!("Skipping integration test: Cargo.toml not found at {}", root.display());
        return;
    }

    let workspace = discovery.discover(root).expect("Should discover real repo workspace");
