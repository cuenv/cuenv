...
fn create_task_with_project_ref(
    ...
    Task {
        ...
        depends_on: deps.into_iter().map(String::from).collect(),
        description: Some(format!("Test task {}", name)),
        inputs: vec![super::super::Input::Project(super::super::ProjectReference {
...
fn test_synthetic_hook_task_naming() {
    ...
    let hook_task2 = create_task("bun.hooks.beforeInstall[1]", vec!["bun.hooks.beforeInstall[0]"], vec![]);
    ...
    graph.add_task("bun.hooks.beforeInstall[0]", hook_task1).unwrap();
    graph.add_task("bun.hooks.beforeInstall[1]", hook_task2).unwrap();
...
fn test_synthetic_hook_with_named_matcher() {
    ...
    graph.add_task("bun.hooks.beforeInstall.projen[0]", hook1).unwrap();
    graph.add_task("bun.hooks.beforeInstall.projen[1]", hook2).unwrap();
...
fn test_synthetic_hook_sequential_matcher() {
    ...
    graph.add_task("bun.hooks.beforeInstall.codegen[0]", hook1).unwrap();
    graph.add_task("bun.hooks.beforeInstall.codegen[1]", hook2).unwrap();
    graph.add_task("bun.hooks.beforeInstall.codegen[2]", hook3).unwrap();
    graph.add_task("bun.install", install).unwrap();
    graph.add_dependency_edges().unwrap();
...
fn test_multiple_hook_steps_in_order() {
    ...
    graph.add_task("bun.hooks.beforeInstall.projen[0]", step1_a).unwrap();
    graph.add_task("bun.hooks.beforeInstall.projen[1]", step1_b).unwrap();
    ...
    let pos_step0 = names.iter().position(|&n| n == "bun.hooks.beforeInstall[0]").unwrap();
    let pos_step1a = names.iter().position(|&n| n == "bun.hooks.beforeInstall.projen[0]").unwrap();
    let pos_step1b = names.iter().position(|&n| n == "bun.hooks.beforeInstall.projen[1]").unwrap();
    let pos_step2 = names.iter().position(|&n| n == "bun.hooks.beforeInstall[2]").unwrap();
...
fn test_workspace_setup_chain() {
    ...
    assert_eq!(
        names,
        vec!["bun.hooks.beforeInstall[0]", "bun.install", "bun.setup", "dev"]
    );
}
...
fn test_hook_with_cross_project_task_ref() {
    ...
    graph.add_task("projen-generator::install", external_install).unwrap();
    graph.add_task("projen-generator::types", external_types).unwrap();
    graph.add_task("website.bun.hooks.beforeInstall[0]", hook).unwrap();
    ...
    let pos_ext_install = names.iter().position(|&n| n == "projen-generator::install").unwrap();
    let pos_ext_types = names.iter().position(|&n| n == "projen-generator::types").unwrap();
    let pos_hook = names.iter().position(|&n| n == "website.bun.hooks.beforeInstall[0]").unwrap();
    let pos_install = names.iter().position(|&n| n == "website.bun.install").unwrap();
...
fn test_multiple_projects_with_hooks() {
    ...
    graph.add_task("projectA::bun.hooks.beforeInstall[0]", a_hook).unwrap();
    graph.add_task("projectA::bun.install", a_install).unwrap();
    graph.add_task("projectB::npm.hooks.beforeInstall[0]", b_hook).unwrap();
...
fn test_hook_with_afterinstall() {
    ...
    graph.add_task("bun.hooks.beforeInstall[0]", before_hook).unwrap();
    graph.add_task("bun.install", install).unwrap();
    graph.add_task("bun.hooks.afterInstall[0]", after_hook).unwrap();
    graph.add_task("bun.setup", setup).unwrap();
    graph.add_dependency_edges().unwrap();
...
fn test_build_for_task_includes_cross_project_deps() {
    ...
    tasks.tasks.insert(
        "deploy".to_string(),
        TaskDefinition::Single(Box::new(create_task("deploy", vec!["external::build"], vec![]))),
    );
...
fn test_build_for_task_with_hook_chain() {
    ...
    tasks.tasks.insert(
        "dev".to_string(),
        TaskDefinition::Single(Box::new(create_workspace_task("dev", vec!["bun.setup"], vec!["bun"]))),
    );
