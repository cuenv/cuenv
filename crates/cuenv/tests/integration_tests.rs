...
// ============================================================================
// Owners command integration tests
// ============================================================================
...
fn test_owners_sync_dry_run() {
    ...
    match result {
        ...
        Err(e) => panic!("Failed to run cuenv owners sync --dry-run: {e}"),
    }
}

#[test]
fn test_owners_sync_dry_run_sections() {
    let test_path = get_owners_example_path();
    let result = run_cuenv_command(&[
        "owners",
        "sync",
...
fn test_owners_check_no_file() {
    ...
    match result {
        Ok((stdout, stderr, success)) => {
            ...
            assert!(
                combined.contains("not found") || combined.contains("Run 'cuenv owners sync'"),
                "Error should mention file not found or suggest running sync"
            );
        }
        Err(e) => panic!("Failed to run cuenv owners check: {e}"),
    }
}

#[test]
fn test_owners_no_config() {
    ...
    match result {
        ...
        Err(e) => panic!("Failed to run cuenv owners sync: {e}"),
    }
}
