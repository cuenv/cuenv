...
fn test_sync_command_dry_run() {
    ...
    match result {
        Ok((stdout, stderr, success)) => {
            ...
            assert!(
                stdout.contains("Would generate") || stdout.contains(".gitignore"),
...
#[test]
fn test_sync_command_dry_run_shows_content() {
    let test_path = get_test_examples_path();
    let result = run_cuenv_command(&[
        "sync",
        "--path",
    ...
    match result {
        Ok((stdout, _stderr, success)) => {
            assert!(success, "Command should succeed");
            // Should show the generated content header
            assert!(
                stdout.contains("Generated by cuenv") || stdout.contains("---"),
                "Dry run should show file content with header"
            );
        }
        Err(e) => panic!("Failed to run cuenv sync --dry-run: {e}"),
    }
...
fn test_sync_command_help() {
    ...
    match result {
        Ok((stdout, _stderr, _success)) => {
            assert!(
                stdout.contains("sync") || stdout.contains("Generate"),
