...
pub(super) async fn execute_sync_github(request: GithubSyncRequest<'_>) -> Result<String> {
    ...
    let stale_workflows = if hestia_enabled {
        Vec::new()
    } else {
        vec![cuenv_github::workflow::HESTIA_GC_WORKFLOW_FILENAME]
    };
...
fn sync_github_workflow_files(request: GithubWorkflowFilesSyncRequest<'_>) -> Result<String> {
    ...
    if options.check {
        ...
        for filename in stale_workflows {
            if workflows_dir.join(filename).exists() {
                out_of_sync.push(format!("{filename} (stale)"));
            }
        }
        ...
        return if workflows.is_empty() {
            Ok(String::new())
        } else {
            Ok(format!("GitHub: {} workflow(s) in sync", workflows.len()))
        };
    }
    ...
    for filename in stale_workflows {
        ...
        if options.dry_run.is_dry_run() {
            output_lines.push(format!("GitHub: Would remove {filename}"));
            continue;
        }

        std::fs::remove_file(&workflow_path).map_err(|error| {
            cuenv_core::Error::io_with_path(
                "remove stale workflow file",
                workflow_path.clone(),
                error,
            )
        })?;
        output_lines.push(format!("GitHub: Removed {filename}"));
    }
...
mod tests {
    ...
    fn write_stale_hestia_gc_workflow(
        ...
        let workflow_path = workflows_dir.join(cuenv_github::workflow::HESTIA_GC_WORKFLOW_FILENAME);
        std::fs::write(&workflow_path, "# Generated by cuenv\n")?;
        Ok((workflows_dir, workflow_path))
    }
    ...
    #[test]
    fn disabled_hestia_removes_stale_gc_workflow()
    -> std::result::Result<(), Box<dyn std::error::Error>> {
        let temp_dir = tempfile::tempdir()?;
        let (workflows_dir, workflow_path) = write_stale_hestia_gc_workflow(&temp_dir)?;
        let stale_workflows = [cuenv_github::workflow::HESTIA_GC_WORKFLOW_FILENAME];

        let output = sync_github_workflow_files(GithubWorkflowFilesSyncRequest {
            workflows_dir: &workflows_dir,
            workflows: &[],
            stale_workflows: &stale_workflows,
            options: CiSyncOptions {
                dry_run: cuenv_core::DryRun::No,
                check: false,
                provider: None,
            },
        })?;

        assert_eq!(output, "GitHub: Removed cuenv-hestia-cache-gc.yml");
        assert!(!workflow_path.exists());
        Ok(())
    }

    #[test]
    fn disabled_hestia_dry_run_reports_stale_gc_workflow_without_removing_it()
    -> std::result::Result<(), Box<dyn std::error::Error>> {
        let temp_dir = tempfile::tempdir()?;
        let (workflows_dir, workflow_path) = write_stale_hestia_gc_workflow(&temp_dir)?;
        let stale_workflows = [cuenv_github::workflow::HESTIA_GC_WORKFLOW_FILENAME];
        ...
        let output = sync_github_workflow_files(GithubWorkflowFilesSyncRequest {
            ...
            stale_workflows: &stale_workflows,
            options: CiSyncOptions {
                dry_run: cuenv_core::DryRun::Yes,
                check: false,
                provider: None,
            },
        })?;

        assert_eq!(output, "GitHub: Would remove cuenv-hestia-cache-gc.yml");
        assert!(workflow_path.exists());
        Ok(())
    }

    #[test]
    fn disabled_hestia_check_rejects_stale_gc_workflow()
    -> std::result::Result<(), Box<dyn std::error::Error>> {
        let temp_dir = tempfile::tempdir()?;
        let (workflows_dir, workflow_path) = write_stale_hestia_gc_workflow(&temp_dir)?;
        let stale_workflows = [cuenv_github::workflow::HESTIA_GC_WORKFLOW_FILENAME];

        let result = sync_github_workflow_files(GithubWorkflowFilesSyncRequest {
            workflows_dir: &workflows_dir,
            workflows: &[],
            stale_workflows: &stale_workflows,
            options: CiSyncOptions {
        ...
        let Err(error) = result else {
            return Err(std::io::Error::other(
                "check mode unexpectedly accepted a stale Hestia GC workflow",
            )
            .into());
        };
        ...
        assert!(
            error
                .to_string()
                .contains("cuenv-hestia-cache-gc.yml (stale)")
        );
        assert!(workflow_path.exists());
        Ok(())
    }
