...
use cuenv_core::manifest::{Base, Ignore, Project};
use cuenv_core::module::InstanceKind;
...
fn collect_cube_gitignore_patterns(cube: Option<&cuenv_core::manifest::CubeConfig>) -> Vec<String> {
...
/// Convert ignore configuration to `cuenv_ignore::IgnoreFile` configs.
/// Also integrates cube-generated gitignore patterns.
fn convert_ignore_to_files(ignore: Option<&Ignore>, cube_patterns: &[String]) -> Vec<IgnoreFile> {
    let mut files = Vec::new();

    // Process ignore configurations
    if let Some(ignore) = ignore {
        for (tool, value) in ignore {
            ...
            if tool == "git" && !cube_patterns.is_empty() {
                ...
                patterns.extend(cube_patterns.iter().cloned());
            }
    ...
    if !cube_patterns.is_empty() {
        let has_git_ignore = ignore.is_some_and(|i| i.contains_key("git"));
...
fn convert_project_to_ignore_files(manifest: &Project) -> Vec<IgnoreFile> {
    let cube_patterns = collect_cube_gitignore_patterns(manifest.cube.as_ref());
    convert_ignore_to_files(manifest.ignore.as_ref(), &cube_patterns)
}
...
fn convert_base_to_ignore_files(manifest: &Base) -> Vec<IgnoreFile> {
    convert_ignore_to_files(manifest.ignore.as_ref(), &[])
}
...
pub async fn execute_sync_ignore(
    ...
    let (instance, _module_root) = load_instance_at_path(dir_path, package, executor)?;

    // Convert to ignore files (includes cube gitignore patterns for Projects)
    let files = match instance.kind {
        InstanceKind::Project => {
            let manifest: Project = instance.deserialize()?;
            convert_project_to_ignore_files(&manifest)
        }
        InstanceKind::Base => {
            let manifest: Base = instance.deserialize()?;
            convert_base_to_ignore_files(&manifest)
        }
    };
...
mod tests {
    ...
    fn test_collect_cube_gitignore_patterns_empty() {
        let manifest = Project::new("test");
        let patterns = collect_cube_gitignore_patterns(manifest.cube.as_ref());
