...
fn sync_pre_push_hook(
    git_root: &Path,
    hooks: &std::collections::HashMap<String, cuenv_core::hooks::Hook>,
    dry_run: bool,
    check: bool,
) -> Result<String> {
    use std::fs;
...
fn generate_pre_push_script(
    _hooks: &[(&String, &cuenv_core::hooks::Hook)],
) -> String {
...
mod tests {
    ...
    fn test_generate_pre_push_script() {
        let hooks = vec![];
        let script = generate_pre_push_script(&hooks);
