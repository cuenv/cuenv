...
//! - `cuenv owners sync` - Sync CODEOWNERS file from CUE configuration
//! - `cuenv owners check` - Check CODEOWNERS file is in sync with CUE configuration
...
pub async fn execute_owners_check(path: &str, package: &str) -> Result<String> {
    ...
    if !output_path.exists() {
        return Err(cuenv_core::Error::configuration(format!(
            "CODEOWNERS file not found at {}. Run 'cuenv owners sync' to generate it.",
            output_path.display()
        )));
    }
    ...
    if normalize(&current_content) == normalize(&expected_content) {
        ...
        Err(cuenv_core::Error::configuration(format!(
            "CODEOWNERS file is out of sync at {}. Run 'cuenv owners sync' to update it.",
            output_path.display()
        )))
    }
}
