...
fn collect_cube_gitignore_patterns(manifest: &Cuenv) -> Vec<String> {
...
pub async fn execute_sync_ignore(
    ...
    // Convert to ignore files (includes both project ignores and cube gitignore patterns)
    let files = convert_to_ignore_files(&manifest);

    if files.is_empty() {
        tracing::info!("No ignore patterns configured");
        return Ok(
            "No ignore patterns configured. Add an `ignore` field or set `gitignore: true` on cube files.".to_string(),
        );
    }
...
mod tests {
    ...
    fn test_convert_to_ignore_files_with_cube_patterns_only() {
        ...
        let ignore_files = convert_to_ignore_files(&manifest);
