...
mod tests {
    ...
    fn detect_cache_dir_with_home(manager: PackageManager) -> Option<PathBuf> {
        with_var("HOME", Some("/home/test"), || {
            NodeModulesMaterializer::detect_cache_dir(manager)
        })
    }
    ...
    fn test_detect_cache_dir_npm() {
        let cache = detect_cache_dir_with_home(PackageManager::Npm);
    ...
    fn test_detect_cache_dir_pnpm() {
        let cache = detect_cache_dir_with_home(PackageManager::Pnpm);
        assert!(cache.is_some());
    ...
    fn test_detect_cache_dir_bun() {
        let cache = detect_cache_dir_with_home(PackageManager::Bun);
        assert!(cache.is_some());
        let cache_path = cache.unwrap();
    ...
    fn test_detect_cache_dir_yarn_classic() {
        let cache = detect_cache_dir_with_home(PackageManager::YarnClassic);
        assert!(cache.is_some());
        let cache_path = cache.unwrap();
    ...
    fn test_detect_cache_dir_yarn_modern() {
        let cache = detect_cache_dir_with_home(PackageManager::YarnModern);
        assert!(cache.is_some());
        let cache_path = cache.unwrap();
    ...
    fn test_detect_cache_dir_deno() {
        let cache = detect_cache_dir_with_home(PackageManager::Deno);
        assert!(cache.is_some());
        let cache_path = cache.unwrap();
    ...
    fn test_detect_cache_dir_cargo_returns_none() {
        let cache = detect_cache_dir_with_home(PackageManager::Cargo);
