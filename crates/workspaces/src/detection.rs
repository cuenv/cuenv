...
fn detect_yarn_version(lockfile_path: &Path) -> Result<PackageManager> {
    ...
    if has_yarn_modern_metadata(&content) {
        Ok(PackageManager::YarnModern)
    } else if has_yarn_classic_header(&content) {
...
fn has_yarn_modern_metadata(content: &str) -> bool {
    content
        .lines()
        .take(5)
...
fn has_yarn_classic_header(content: &str) -> bool {
...
"#;
    ...
    #[test]
    fn test_detect_yarn_modern_metadata_within_first_five_lines() {
        let temp_dir = create_test_workspace();
        let lockfile_path = temp_dir.path().join("yarn.lock");

        let modern_content = r#"# generated by yarn
# do not edit
