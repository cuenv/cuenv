...
pub fn create_host_functions() -> Vec<Function> {
    ...
    let random_fill = Function::new(
        ...
        |plugin: &mut CurrentPlugin, inputs: &[Val], outputs: &mut [Val], _: UserData<()>| {
            ...
            let offset = wasm_memory_offset(handle.offset())?;
    ...
    let time_op_now = Function::new(
        ...
        |_plugin: &mut CurrentPlugin, _inputs: &[Val], outputs: &mut [Val], _: UserData<()>| {
            let now = unix_time_milliseconds(SystemTime::now());
...
fn wasm_memory_offset(offset: u64) -> Result<i64, extism::Error> {
    i64::try_from(offset).map_err(|_| {
        extism::Error::msg(format!(
            "WASM memory offset {offset} exceeds host function i64 boundary"
        ))
    })
}

fn unix_time_milliseconds(time: SystemTime) -> i64 {
    let duration = time.duration_since(UNIX_EPOCH).unwrap_or_default();
    duration_milliseconds_i64(duration)
}

fn duration_milliseconds_i64(duration: Duration) -> i64 {
...
mod tests {
    ...
    #[test]
    fn test_wasm_memory_offset_rejects_i64_overflow() {
        let max_i64 = u64::try_from(i64::MAX).unwrap();

        assert_eq!(wasm_memory_offset(max_i64).unwrap(), i64::MAX);
        assert!(wasm_memory_offset(max_i64 + 1).is_err());
    }

    #[test]
    fn test_duration_milliseconds_i64_saturates() {
        let max_i64 = u64::try_from(i64::MAX).unwrap();

        assert_eq!(duration_milliseconds_i64(Duration::from_millis(42)), 42);
        assert_eq!(
            duration_milliseconds_i64(
                Duration::from_millis(max_i64).saturating_add(Duration::from_millis(1))
            ),
            i64::MAX
        );
    }
    ...
    #[test]
    fn test_unix_time_milliseconds_clamps_pre_epoch() {
        assert_eq!(unix_time_milliseconds(UNIX_EPOCH), 0);
        assert_eq!(
            unix_time_milliseconds(UNIX_EPOCH - Duration::from_millis(1)),
            0
        );
    }
