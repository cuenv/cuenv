...
impl GitHubActionsEmitter {
    ...
    fn build_permissions(ir: &IntermediateRepresentation) -> Permissions {
        ...
        let has_outputs = ir
            .tasks
            .iter()
            .any(|t| t.outputs.iter().any(|o| o.output_type == OutputType::Orchestrator));
    ...
    fn build_job(&self, task: &Task) -> Job {
        ...
        if !orchestrator_outputs.is_empty() {
            let paths: Vec<String> = orchestrator_outputs.iter().map(|o| o.path.clone()).collect();
    ...
    fn serialize_workflow(workflow: &Workflow) -> EmitterResult<String> {
        let yaml =
            serde_yaml::to_string(workflow).map_err(|e| EmitterError::Serialization(e.to_string()))?;
...
mod tests {
    ...
    fn test_simple_workflow() {
        let emitter = GitHubActionsEmitter::new().without_nix().without_cuenv_build();
    ...
    fn test_workflow_with_cachix() {
        let emitter = GitHubActionsEmitter::new().with_nix().with_cachix("my-cache");
        let ir = make_ir(vec![make_task("build", vec!["cargo", "build"])]);
