...
- Services are partial: `down` can request whole-session shutdown from the persisted `cuenv up` session, but named-service shutdown still needs supervisor IPC; `logs --follow` tails persisted session logs until the `cuenv up` controller exits; `restart` queues persisted restart requests that running supervisors consume.
