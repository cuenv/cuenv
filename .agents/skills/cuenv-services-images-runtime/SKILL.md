...
- Container runtime support is schema-only until the matrix says otherwise. RFC-0007 drops `#ContainerRuntime` from the `#Runtime` union; Dagger requires `#DaggerRuntime` with `image` or `from`.
