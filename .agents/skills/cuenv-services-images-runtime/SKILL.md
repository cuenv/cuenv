...
- `#ContainerImage` is schema-visible, but `cuenv build` only lists image definitions. Selected image builds fail fast until an execution backend exists.
- Services are partial: `up` honors service-to-service dependencies, includes service deps for selected starts, runs task dependencies before service startup, and recognizes image dependencies before failing fast until image build backends exist; `down` can request whole-session shutdown or named-service stop requests from the persisted `cuenv up` session; `logs --follow` tails persisted session logs until the `cuenv up` controller exits; `restart` queues persisted restart requests that running supervisors consume.
...
- "Create a production image build with cuenv build." Explain current schema-only build status and that selected build requests are rejected.
