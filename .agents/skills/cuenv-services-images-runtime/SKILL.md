...
- `#ContainerImage` is partial: `cuenv build` lists image definitions and builds selected images through the local Docker CLI. Registry builds are pushed with Docker buildx; Dagger execution and downstream output-reference resolution remain incomplete.
