...
- Container runtime support is schema-only until the matrix says otherwise. RFC-0007 does not treat `#ContainerRuntime` as a Dagger opt-in; Dagger requires `#DaggerRuntime` with `image` or `from`.
...
- `#ContainerImage` is partial: `cuenv build` lists image definitions and builds selected images. An image sets either `context` (Dockerfile, built with docker/buildx — registry builds pushed with buildx) or `installable` (Nix flake output, built with `nix build` then delivered via `docker load`/`tag`/`push`); the two are mutually exclusive. Multi-arch Nix images, Dagger execution, and downstream output-reference resolution remain incomplete. The intended Dagger v1 path (runtime form executes, one session per graph, Dockerfile builds via Dagger) is RFC-0007; do not claim it is implemented until the matrix says so.
- Services are partial: `up` honors service-to-service dependencies, includes service deps for selected starts, runs task dependencies before service startup, and recognizes image dependencies before failing fast until image build backends are wired into service startup; `down` can request whole-session shutdown or named-service stop requests from the persisted `cuenv up` session; `logs --follow` tails persisted session logs until the `cuenv up` controller exits; `restart` queues persisted restart requests that running supervisors consume.
...
- "Create a production image build with cuenv build." Explain current partial Docker build support, including registry push behavior, and note that Dagger execution and downstream output-reference resolution remain incomplete. RFC-0007 is the intended Dagger v1 image path.
