...
- `config.ci.cuenv: {source: "nix", version: "self"}` bootstraps the checked-out cuenv in every emitted job that needs cuenv. The release workflow's first `build.cuenv` jobs order downstream work, but they do not share a `result/bin/cuenv` artifact with `publish.cue`, docs, or cargo jobs. Each job is isolated and relies on its own Nix build plus Linux Namespace `/nix` cache restore; inspect completed job logs before deciding whether a repeated `Build cuenv (nix)` step was a real rebuild or a cache/substitution hit.
