...
- `pipelines[*].continueOnError` (default `false`): when `true`, the per-project orchestrator does not abort on the first failure; dependents of the failing task become `task.skipped` events with a `DependencyFailed { dep }` reason while independent siblings keep running. The pipeline still exits non-zero overall if any task failed — it controls scheduling, not success.
- Per-project task execution is parallel (bounded `JoinSet`, cap `CI_MAX_PARALLEL = 4` in `crates/ci/src/executor/orchestrator.rs`). Ready-but-capped tasks emit `task.queued` events so renderers can show them.
