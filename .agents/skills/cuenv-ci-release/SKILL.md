...
- Hestia cache support is `ci.provider.github.hestia` plus `contributors.#Hestia`. It emits `Mic92/hestia@fb239a2f72d4b6e26eec5425f289dea23b27a527` after `nix.install`, pins the downloaded binary to v2.0.0, enables `upstream-cache-filter`, and sets `drain-timeout` to 900 seconds. Ordinary build jobs use the runner-injected Actions cache token and do not need `actions: write`. Concurrent jobs in one workflow run merge their closures into the same root by union. When any GitHub pipeline enables Hestia, `cuenv sync ci` also emits one repository-wide `cuenv-hestia-cache-gc.yml` workflow that runs daily, serializes GC, supports manual dry runs, stays on the default branch, and grants only that maintenance workflow `actions: write`.
...
- `config.ci.cuenv: {source: "nix", version: "self"}` builds the checked-out cuenv for emitted jobs that need cuenv. Expanded GitHub workflows skip `build.cuenv` entirely for direct Nix jobs. Orchestrated jobs consume a runner-specific `cuenv` artifact from `build.cuenv` and then run later setup tasks such as 1Password. Linux jobs still restore the Namespace `/nix` cache before Nix setup; macOS jobs skip the Namespace cache action.
