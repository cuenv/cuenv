...
- Hestia cache support is `ci.provider.github.hestia` plus `contributors.#Hestia`. It emits `Mic92/hestia@fb239a2f72d4b6e26eec5425f289dea23b27a527` after `nix.install`, pins the downloaded binary to v2.0.0, enables `upstream-cache-filter`, and sets `drain-timeout` to 900 seconds. Ordinary build jobs use the runner-injected Actions cache token and do not need `actions: write`. Concurrent jobs in one workflow run merge their closures into the same root by union. When any GitHub pipeline enables Hestia, `cuenv sync ci` also emits one repository-wide `cuenv-hestia-cache-gc.yml` workflow that runs daily, serializes GC, supports manual dry runs, stays on the default branch, and grants only that maintenance workflow `actions: write`. When no generated GitHub pipeline enables Hestia, normal sync removes the GC workflow and check mode reports it as stale.
...
- Namespace cache volumes use private forks with last-write-wins commits and may serve stale generations. Parallel jobs sharing one Namespace volume do not union their Nix closures, so a mounted Namespace cache is not proof that a derivation built by a sibling job is present. Prefer Hestia when parallel GitHub jobs need a coherent union; treat Namespace as an opportunistic local store if both are used.
- `config.ci.cuenv: {source: "nix", version: "self"}` builds the checked-out cuenv for emitted jobs that need cuenv. Expanded GitHub workflows skip `build.cuenv` entirely for direct Nix jobs. Orchestrated jobs consume a runner-specific `cuenv` artifact from `build.cuenv` and then run later setup tasks such as 1Password.
...
- Same-project task-output inputs retain their producer identity when manifest expansion converts them into concrete mapped paths. CI compilation uses that provenance to emit orchestrator artifact downloads, while the mapped source/destination still drives local hashing and execution-root materialization. `dependsOn` alone orders jobs and never transfers outputs.
...
- Cache eligibility skips emit `task.cache_skipped` events with a structured `CacheSkipReason` (`EmptyInputs`, `NonPathRef`, `NoResolvedInputs`, `RuntimeEnv`, `Disabled { reason }`, `NeverMode`, `HasherRootMismatch`, `HashFailed { reason }`). Preserve the input hasher's error text in `HashFailed`; a bare "hashing failed" is not actionable when directory isolation turns resolution failures into configuration errors.
