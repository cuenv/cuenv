...
- Cucumber BDD coverage keeps the root runner in `crates/cuenv/tests/bdd.rs`; shared world state and env/task/hook step definitions live under `crates/cuenv/tests/bdd_support/`. Keep that split when adding hook, shell integration, env, or task behavior scenarios instead of growing one oversized BDD file.
