...
- Cucumber BDD coverage keeps the root runner in `crates/cuenv/tests/bdd.rs`; shared world state and env/task/hook step definitions live under `crates/cuenv/tests/bdd_support/`. Keep world initialization, project fixture writes, CUE module setup, env-step tables, and task command execution on fallible `StepResult` paths instead of reintroducing broad unwrap/expect allowances, and keep that split when adding hook, shell integration, env, or task behavior scenarios instead of growing one oversized BDD file.
