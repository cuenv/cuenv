...
- `crates/hooks/src/state/` for persistent hook execution state, marker files, cleanup, execution hashes, and integer duration display formatting; hook result recording should borrow the result and clone only for persisted state. `crates/hooks/src/executor.rs` owns hook execution orchestration and saturating elapsed-millisecond conversion; `crates/hooks/src/executor/source_environment.rs` owns source-hook environment capture. Supervisor integration tests should scope `CUENV_EXECUTABLE` with `temp_env`, not unsafe process-wide mutation, and route skip diagnostics through tracing instead of raw stderr.
- Cucumber BDD coverage keeps the root runner in `crates/cuenv/tests/bdd.rs`; shared world state and env/task/hook step definitions live under `crates/cuenv/tests/bdd_support/`. Keep world initialization, project fixture writes, CUE module setup, env-step tables, task command execution, hook status polling, approval setup, shell execution, and hook failure assertions on fallible `StepResult` paths instead of reintroducing broad unwrap/expect/print allowances or warning-based skipped assertions, and keep that split when adding hook, shell integration, env, or task behavior scenarios instead of growing one oversized BDD file.
...
- `crates/cuenv/src/commands/hooks.rs` for CLI hook command orchestration; `crates/cuenv/src/commands/hooks/status.rs` for status/starship rendering; `crates/cuenv/src/commands/hooks/shell.rs` for `cuenv shell init` snippets. Hook-backed export wait progress lives in `crates/cuenv/src/commands/export/hooks.rs` and should use its stderr writer helper instead of raw print macros. Export performance regression tests should keep command execution behind shared `Result` helpers instead of reintroducing file-level unwrap/expect allowances.
...
- Use `schema.#AwsSecret` for AWS Secrets Manager references.
- Treat `schema.#GcpSecret` and `schema.#VaultSecret` as schema-only unless the current matrix changes.
