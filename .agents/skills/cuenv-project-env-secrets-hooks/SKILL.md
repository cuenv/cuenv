...
- Hook syntax-error integration tests should return `Result` from filesystem and command setup, use `Command::new(env!("CARGO_BIN_EXE_cuenv"))`, and report unexpected exits as errors instead of reintroducing file-level unwrap/expect allowances.
