...
- Trace acceptance tests in `crates/cuenv/tests/trace_acceptance.rs` should return `Result` from binary/root discovery, dry-run execution, DAG JSON parsing, and task lookup helpers instead of reintroducing file-level unwrap/expect, print suppressions, or unused tracing helper modules.
