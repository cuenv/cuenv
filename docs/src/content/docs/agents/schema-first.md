...
- Use `schema.#GcpSecret` for Google Cloud Secret Manager; use `schema.#AwsSecret` and `schema.#VaultSecret` only with a status note that runtime resolvers are not registered by default.
