...
- [ ] Keep the existing per-directory banner and border path bounded,
...
- [ ] Add a command palette for Cuenv task discovery and launch only after the
  terminal action catalogue is stable. A task should run in an explicit pane or
  tab with clear cancellation and exit semantics.
