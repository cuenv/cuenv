...
- **`crates/gpui_ghostty_terminal` is a local patch.** Cuetty carries a small app-local copy of the upstream terminal view so native text selection converts GPUI window mouse coordinates into terminal-local coordinates before calculating rows and columns. The local view keeps GPUI entity-input/IME handling in `view/input_handler.rs` so composition state stays separate from viewport, mouse, and render orchestration. Drop this copy once upstream includes the coordinate fix.
