...
- **`crates/gpui_ghostty_terminal` is a local patch.** Cuetty carries a small app-local copy of the upstream terminal view so native text selection converts GPUI window mouse coordinates into terminal-local coordinates before calculating rows and columns. The local view keeps GPUI entity-input/IME handling in `view/input_handler.rs`, text shaping/prepaint/paint work in `view/element.rs`, coordinate helpers in `view/geometry.rs`, and link detection in `view/links.rs` so composition and rendering state stay separate from viewport and mouse orchestration. The app shell keeps tab and pane layout-tree operations in `ui/layout.rs`, away from root rendering and process launch code. Drop this copy once upstream includes the coordinate fix.
