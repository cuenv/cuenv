...
- **No Dagger execution yet.** Builds run through the local Docker CLI; the Dagger backend for images is not implemented. [RFC-0007](/decisions/rfcs/rfc-0007-dagger-v1-native-runtime/) is the intended Dockerfile-via-Dagger path, only after an explicit Dagger opt-in.
