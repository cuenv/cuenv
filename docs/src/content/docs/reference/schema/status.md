...
- Secrets: `#ExecSecret`, feature-gated `#OnePasswordRef`, feature-gated `#InfisicalSecret`, and feature-gated `#GcpSecret` are implemented; `#AwsSecret` and `#VaultSecret` are schema-only until runtime resolvers are registered.
